...
function requireModule(mdl, filepath) {
  if (!mdl) {
...
function createValidatorsForFunction(userModule, moduleAnnotations, validators, item) {
  if (!moduleAnnotations[item] || !Array.isArray(moduleAnnotations[item].param)) {
    return validators;
  }
  
  // Parse param args
  const params = coalesceParams(moduleAnnotations[item].param.map(parseDocblockParam));
